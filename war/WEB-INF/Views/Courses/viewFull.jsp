...
<%
	List<Course> res = (List<Course>) request.getAttribute("showFull");
							...
							<tbody>
								<%
									for (int idx = 0; idx < res.size(); idx++) {
										Course t = res.get(idx);
								%>
								<tr>
									<td><%=t.getId()%></td>
									<td><%=t.getName()%></td>
									<td><%=t.getDepartament()%></td>
									<td><%=t.getLevelId()%></td>
									<td><%=t.getTeacher()%></td>
									<td><%=t.getDate()%></td>
									<td><a href="/course/view?ID=<%=t.getId()%>">View</a><a
										href="/course/edit?ID=<%=t.getId()%>">Edit</a><a
										href="/course/delete?ID=<%=t.getId()%>">Delete</a></td>
								</tr>
								<%
									}
								%>
